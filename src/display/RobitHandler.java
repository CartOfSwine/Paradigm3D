...
public class RobitHandler {
	public static RobitEnt[][] populateRobits(World sim, AssetManager assetManager, RobitEnt[][] robits, Node rootNode) {
		Mesh robitShape = new Sphere(10,10,0.3f); //reuse the Mesh object
	...
	public static void updateRobits(RobitEnt[][] robits, final int STEP_TIME, Node rootNode, int tickNum) {
		for(int i = 0; i < robits.length; i++) {
			for(int j = 0; j < robits[i].length; j++) {
				if(robits[i][j].update(STEP_TIME,tickNum)) 
